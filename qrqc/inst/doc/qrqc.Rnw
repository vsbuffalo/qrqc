...
plotted also, as in \ref{fig:BasePlot-prop}.
...
position, as in \ref{fig:Entropy}. \texttt{kmerEntropyPlot} will
...
kmerEntropyPlot(list("low contamination"=s.fastq, "random uniform bases"=s.rand))
@
...
in the plot produced by \texttt{kmerKLPlot}, as in \ref{fig:KmerKL}.
...
K-L divergence term plot, as in \ref{fig:KmerKLRand}
...
kmerKLPlot(list("less contaminated"=s.fastq, "random"=s.rand, "highly contaminated"=s.contam))
@
...
\texttt{coord\_flip()}, as in \ref{fig:Coord}.
...
accessor functions \texttt{getBase}, \texttt{getBaseProps},
\texttt{getQual}, \texttt{getSeqlen}, and \texttt{getMCQual} return
data frames that can then be used directly with \Rpackage{ggplot2}.
