...
\item \texttt{base.freqs}: a dataframe containing the frequency counts
  of each base.
\item \texttt{base.props}: a ``long'' dataframe, with each base
  proportion and position by row.
...
(both MIT License) which this package uses. More on these header files
can be found at
