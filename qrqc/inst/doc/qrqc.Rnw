...
\begin{abstract}
  ...
  Quality Control) package is an fast and extensible package that
...
This produces a \texttt{FASTQSummary} object, which inherits from the
...
``\emph{seqfilename}-report.html''.

\section{Working with the FASTQSummary and FASTASummary classes}
\Rpackage(qrqc) provides the \texttt{FASTQSummary} and
\texttt{FASTASummary} objects for users to build functions and
applications around. Both inherit from \texttt{SequenceSummary}, which
should not be used directly.

\texttt{FASTASummary} provides the same slots as
\texttt{FASTQSummary}, except the latter provides additional slots for
quality information. Both contain: 

\begin{itemize}
\item \texttt{filename}: the filename of the file read and summarized with \texttt{readSeqFile}.
\item \texttt{base.freqs}: a dataframe containing position by row, and frequencies of each base by column.
\item \texttt{base.props}: a ``long'' dataframe, with each bases proportion and position by row.
